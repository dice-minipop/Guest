...
export function SpaceReservationSheetContent({
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      <div className="flex shrink-0 items-center justify-between border-b border-neutral-200 px-5 pt-12 dark:border-neutral-700">
        <p className="typo-h2 text-dice-black dark:text-white">희망 대여 기간 선택</p>
        <button
          type="button"
          onClick={onClose}
          aria-label="닫기"
        ...
        {monthsToRender.map(({ year: y, month: m }) => {
          ...
          return (
            <section key={`${y}-${m}`} className="py-12" aria-label={`${y}년 ${m + 1}월`}>
              <h3 className="typo-body1 mb-3 font-medium text-dice-black dark:text-white">
                {y}년 {m + 1}월
              </h3>
                    ...
                    <button
                      ...
                      className={[
                        ...
                        !isStart &&
                          ...
                          !isInRange &&
                          !isDisabled &&
                          "text-gray-dark hover:bg-neutral-100 dark:text-white dark:hover:bg-neutral-700",
                        (isReserved || isPast) &&
                          "cursor-not-allowed text-gray-light dark:text-neutral-500",
                        isInRange && "text-gray-dark dark:text-white",
                      ]
                        .filter(Boolean)
                        .join(" ")}
                    >
                      {isInRangeOrEnds && (
                        <span
                          className={`absolute top-1/2 z-0 h-[65%] -translate-y-1/2 bg-neutral-100 dark:bg-neutral-500 ${
      ...
      <div className="flex shrink-0 gap-12 border-t border-neutral-200 px-5 py-16 dark:border-neutral-700">
        <div className="flex flex-1 gap-12">
          <button
            type="button"
            onClick={clearDate}
            className="rounded-lg border border-neutral-300 p-16 text-sm font-medium text-neutral-700 transition-colors hover:bg-neutral-50 dark:border-neutral-600 dark:text-neutral-300 dark:hover:bg-neutral-800"
          >
            날짜 초기화
          </button>
          <button
            ...
            className="flex-1 rounded-lg bg-black p-16 text-sm font-medium text-white transition-colors hover:opacity-90 disabled:opacity-40 dark:bg-white dark:text-black dark:disabled:opacity-40"
