  ...
  return (
    <div className="min-h-screen bg-bg-light-gray pb-[72px]">
      <BackHeader
        ...
        rightSlot={
          <button
            ...
            aria-label="채팅방 신고"
          >
            <SirenIcon className="size-6" aria-hidden />
          </button>
        }
      />
        ...
        content={
          <div className="overflow-y-auto py-6 px-5 pb-5">
              ...
              {REPORT_REASONS.map((reason) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full rounded-lg border p-4 text-left typo-subtitle3 transition-colors ${
                      selected
            ...
            <button
              ...
              className="typo-button1 mt-6 w-full rounded-lg bg-dice-black py-[15.5px] text-white transition-opacity hover:opacity-90 disabled:opacity-40"
            >
              {reporting ? "처리 중..." : "신고하기"}
            </button>
          </div>
      ...
      >
        ...
        {!isMemberOnlyAllowed ? (
          <p className="typo-body2 text-gray-deep py-1">
            회원 전용 기능입니다. 로그인 후 이용해 주세요.
          </p>
        ) : isLoading ? (
          <p className="typo-body2 text-gray-deep py-1">메시지를 불러오는 중...</p>
        ) : messages.length === 0 ? (
          <p className="typo-body2 text-gray-deep py-1">아직 메시지가 없습니다.</p>
        ) : (
            ...
            {messages.map((msg, index) => {
              ...
              return (
                <div key={msg.id} className="flex flex-col gap-4">
                  {showDateHeader ? (
                    <p className="typo-caption2 py-2 text-center text-gray-dark">
                      {formatDateHeader(msg.createdAt)}
                    </p>
                  ) : null}
                  <div
                    className={`flex w-full flex-col items-end gap-1 ${msg.isLoginUsersMessage ? "items-end" : "items-start"}`}
                  >
      ...
      >
        <form onSubmit={handleSend} className="flex items-end gap-0.5">
          <input
            type="text"
            value={content}
            onChange={(e) => setContent(e.target.value)}
            placeholder="메시지를 입력하세요"
            className="typo-body2 min-h-10 flex-1 rounded-lg border border-neutral-200 bg-neutral-100 px-1 py-0.5 text-dice-black placeholder:text-gray-deep focus:border-dice-black focus:outline-none"
            aria-label="메시지 입력"
            disabled={sending}
          />
          <button
            ...
            className="typo-subtitle3 h-10 shrink-0 rounded-lg bg-dice-black px-1 text-white transition-opacity hover:opacity-90 disabled:opacity-40"
          >
            전송
          </button>
        </form>
